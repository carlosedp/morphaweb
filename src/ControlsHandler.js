...
export default class ControlsHandler {
    constructor(morphaweb) {
        ...
        this.createFadeInRegionButton = document.getElementById("create-fade-in-region")
        ...
        this.createFadeInRegionButton.addEventListener('click', this.createFadeInRegion)
    ...
    createCropRegion = () => {
        try {
            ...
            this.cropRegion.on('update-end', () => {
                this.showMessage(`Crop region: ${this.formatTime(this.cropRegion.start)} - ${this.formatTime(this.cropRegion.end)}`);
            });
    ...
    cropToSelection = () => {
        try {
            ...
            if (startTime >= endTime) {
                this.showMessage("Invalid crop region: start time must be before end time");
                return;
            }
            ...
            const croppedBuffer = audioContext.createBuffer(channelCount, croppedLength, sampleRate);
            ...
            this.showMessage(`Audio cropped from ${this.formatTime(startTime)} to ${this.formatTime(endTime)}`);
    ...
    createFadeInRegion = () => {
        try {
            ...
            this.fadeInRegion.on('update-end', () => {
                this.showMessage(`Fade-in region: ${this.formatTime(this.fadeInRegion.start)} - ${this.formatTime(this.fadeInRegion.end)}`);
            });

            this.showMessage(`Fade-in region created. Drag edges to adjust fade length.`);
    ...
    createFadeOutRegion = () => {
        try {
            ...
            this.fadeOutRegion.on('update-end', () => {
                this.showMessage(`Fade-out region: ${this.formatTime(this.fadeOutRegion.start)} - ${this.formatTime(this.fadeOutRegion.end)}`);
            });

            this.showMessage(`Fade-out region created. Drag edges to adjust fade length.`);
    ...
    applyFades = () => {
        try {
            ...
            if (!this.fadeInRegion && !this.fadeOutRegion) {
                this.showMessage("No fade regions to apply. Create fade-in or fade-out regions first.");
                return;
            }
            ...
            const audioContext = this.morphaweb.wavesurfer.backend.ac;
            const fadedBuffer = audioContext.createBuffer(channelCount, length, sampleRate);
            ...
            for (let channel = 0; channel < channelCount; channel++) {
                ...
                // Apply fade-in
                if (this.fadeInRegion) {
                    const fadeInStartSample = Math.floor(this.fadeInRegion.start * sampleRate);
                    const fadeInEndSample = Math.floor(this.fadeInRegion.end * sampleRate);
                ...
                // Apply fade-out
                if (this.fadeOutRegion) {
                    const fadeOutStartSample = Math.floor(this.fadeOutRegion.start * sampleRate);
                    const fadeOutEndSample = Math.floor(this.fadeOutRegion.end * sampleRate);

...
async function computeOnsets() {
  ...
  const Onsets = new OnsetsWASM.Onsets(
    alpha,
    5,
    self.params.sampleRate / self.params.hopSize,
    0.02,
  );

  const onsetPositions = Onsets.compute(
    odfMatrix,
    self.params.odfsWeights,
  ).positions;
  ...
  if (onsetPositions.size() == 0) {
    ...
    const positions = essentia
      .vectorToArray(onsetPositions)
      .map((pos) => Math.max(0, pos));
    ...
    switch (type) {
      case "updateParams":
        if (params) {
          ...
          const needRecomputeODF = !arraysEqual(
            self.params.odfs,
            prevParams.odfs,
          );
